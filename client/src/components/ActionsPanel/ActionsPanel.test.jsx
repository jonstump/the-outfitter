...
describe("ActionsPanel status messaging", () => {
  ...
  it("keeps both regions mounted after a message is cleared", () => {
    const { container, store } = renderPanel();
    act(() => store.dispatch(uiActions.setMessage("Share code copied to clipboard.")));
...
describe("share code export field", () => {
  it("always shows a live, populated code — no button press needed to generate one", () => {
    ...
    const field = screen.getByLabelText("Your loadout code");
  ...
  it("copies the exact field value to the clipboard on Copy", async () => {
    const writeText = vi.fn().mockResolvedValue(undefined);
    Object.defineProperty(navigator, "clipboard", { value: { writeText }, configurable: true });
    try {
      renderPanelWith({ name: "Clip me" });
      const code = screen.getByLabelText("Your loadout code").value;
...
describe("share code import field", () => {
  it("Load is disabled until something is pasted", () => {
    ...
    fireEvent.change(screen.getByLabelText("Paste a loadout code to load it"), { target: { value: "x" } });
  ...
  it("loads a pasted code exported from another build, replacing the current one", () => {
    ...
    fireEvent.change(screen.getByLabelText("Paste a loadout code to load it"), { target: { value: code } });
  ...
  it("submits on Enter, not just the Load button", () => {
    ...
    const field = screen.getByLabelText("Paste a loadout code to load it");

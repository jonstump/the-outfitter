...
function attempt({ slotMax, upBudgetOn, upBudget }) {
  ...
  let upSpent = traits.reduce((a, i) => a + TRAITS[i][1], 0);
  if (upSpent > upCap) {
    traits.length = 0;
    upSpent = 0;
  }
  const pool = TRAITS.map((t, i) => i).filter((i) => i !== QM);
  const nT = Math.min(RANDOM_TRAIT_COUNT, pool.length);
  for (let k = 0; k < nT && pool.length; k++) {
    const p = pool.splice(Math.floor(Math.random() * pool.length), 1)[0];
    if (upSpent + TRAITS[p][1] <= upCap) {
      traits.push(p);
      upSpent += TRAITS[p][1];
    }
  }
  ...
  const w1c = WEAPONS.map((w, i) => i).filter((i) => WEAPONS[i][1] <= cap - 1);
  const i1 = w1c[Math.floor(Math.random() * w1c.length)];
  const rem = cap - WEAPONS[i1][1];
  const w2c = WEAPONS.map((w, i) => i).filter((i) => WEAPONS[i][1] <= rem);
  const i2 = w2c.length ? w2c[Math.floor(Math.random() * w2c.length)] : null;
  const mkAmmo = (i) =>
    Math.random() < 0.3 && AMMO[WEAPONS[i][3]].length ? Math.floor(Math.random() * AMMO[WEAPONS[i][3]].length) : -1;

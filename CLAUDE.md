...
| SPEC-0005 | SPEC-0005: Desktop Distribution | 16 | `PVT_kwHOA4k1ys4Bg12s` | **closed** |
...
**SPEC-0005 was absent on purpose through 2026-08-18, and its absence up to that point was NOT a gap.** Desktop Distribution carried `status: blocked` — no project, no epic, no stories *by decision* — and during that window it was not to be reported as unplanned work, missing coverage, or drift by `/sdd:audit`, `/sdd:check`, or any ticket review. The gate was **product completeness, not technical readiness** — an installed desktop build missing catalog items, carrying known-wrong data, or unable to express loadouts the game allows would have been half-baked, and desktop users cannot be redeployed out of it. **Seven gates as of 2026-08-15**, all clear as of 2026-08-17 (history below). Gates clearing was a necessary condition, not the decision itself: **the owner made the deliberate call on 2026-08-18, `status:` flipped to `approved` via `/sdd:status`, and `/sdd:plan SPEC-0005` is now expected to run.** This paragraph and the one below are kept as the historical record of why the absence was deliberate, not as a standing prohibition — once a SPEC-0005 project exists, add its row to the table above like every other spec.
...
3. The data-audit remediation worked down — **clear as of 2026-08-16, 42 of 42 closed** (the #351–#394 sweep). History: 25 of 42 closed by that point (#355, #357, #358, #359 and #360 closed 2026-08-16 via PRs #437/#433/#434/#435/#436; #361, #365, #371, #372, #380, #382, #384, #385, #387 and #389 closed 2026-08-16 via PRs #442/#447/#444/#438/#448/#440/#441/#439/#449/#446; #367 and #373 closed 2026-08-16 via PRs #443/#445, both needing a fix-up round after review — a comment-placement CI failure and a missing ADR-0014 citation, respectively — before merging; #363 closed 2026-08-16 via PR #452, also gate 4's third blocker; #356 and #366 closed 2026-08-16, no PR — both were already superseded by merged ammo-epic work rather than needing new code: #356 asked the UI to stop *asserting* per-weapon ammo compatibility it didn't model, and closed itself on inspection by its own stated condition ("close it as obsolete if #338 lands first") once #344 gave the UI the real thing; #366 asked `AGENTS.md` to stay in step as the wire format and ammo model changed, and its four-item checklist (`FORMAT_VERSION`, the `AMMO` foot-gun section, the scrape-provenance line) was already fully satisfied by #340/#341/#345's PRs — verified directly against the file's current text, not assumed from the checklist; joining #351–#354 and #376), plus a further 14 closed 2026-08-16 in one `/sdd:review` batch (#362/#368/#370/#375/#377/#378/#379/#383/#386/#390/#391/#392/#393/#394 via PRs #487/#475/#478/#482/#479/#477/#476/#480/#481/#483/#484/#471/#485/#486 — 39 of 42 total), **#369 and #374 are now also closed** — both PRs (#474, #473) came back with Changes Requested (#474 fixed six stale trait-count references but introduced its own stale line-number citation; #473's otherwise well-verified ADR cited an audit doc that existed only on an orphaned, never-merged branch), were fixed and re-verified, then merged: #474's citation now names `MAX_TRAITS` rather than a line number; #473 brought in the 11-file adversarial QA report (plus the review prompt it references) from `claude/adversarial-qa-review-prompt-p5pe21` as pure file additions — that branch as a whole predates the rebrand/icon/SPEC-0009/SPEC-0010 and is far too old to merge wholesale, so only the audit docs themselves were cherry-picked, content-checked against what the ADR quotes. **#388 is now closed too**, via PR #488 (merged 2026-08-16) — **the sweep is fully closed, 42 of 42.** Its first `/sdd:work` dispatch had silently orphaned (tool call reported success but no agent transcript was ever written, and it sat idle 70+ minutes before being caught by comparing elapsed time against sibling agents in the same batch); re-dispatched cleanly. #488 labeled the two synthetic `obtainable: false` test fixtures as deliberately testing a currently-unreachable branch and added a domain-pinning regression test (`HUNTERS obtainable domain is exactly {true, null}`) — independently mutation-tested by setting one hunter to `false` and confirming the new test catches it. Two of #482's and #475's merges hit real (not just file-overlap) conflicts against sibling PRs merged earlier in the same batch — both were concurrent-insertion or already-resolved-elsewhere conflicts, resolved by hand, re-verified with a full test run, and pushed before merging. **The granularity question the spec asked to settle before burning down further was never formally decided, and by 2026-08-16 it became moot rather than resolved** — the severity-reading subset (blockers + should-fix + #366, 13 issues) reached 13/13 closed the same day the strict reading (all 42) reached 41/42, so no build was ever scheduled against one reading and found wanting under the other. Recorded as an overtaken risk in `desktop-distribution/spec.md`, not a validated shortcut — the next slower-moving gate with this shape should still settle it deliberately.
4. Equipment drag and drop settled — **clear as of 2026-08-17** (all five *original* blockers closed 2026-08-15/16: #352/#353/#419 via PR #430, #382 via PR #440, #363 via PR #452 — its "fix" turned out to be correcting the issue's own stale premise, since #353 had already wired `slotMax` into the live `equipOverCapacity` path rather than leaving it dead code as #363 assumed, so the actual fix was deduping its blocked-cell count to match `hasFreeCell` instead — and #241 via PR #454, another stale-premise correction: #241 was filed against unimplemented SPEC-0006 and proposed a movement threshold plus a Delete/Backspace key, but the shipped implementation already resolved all three gesture collisions differently — via a dedicated remove button, cleanly separated Enter/Space semantics, and pointer-capture event retargeting — independently verified against the live component code and the W3C Pointer Events spec, not just the PR's narrative). Correction (`/sdd:audit`, 2026-08-16): a prior note here claimed a stale "designated remove target" phrase still survived in `spec.md`'s Icon-Only Controls subsection — re-checked directly and the word "designated" was indeed already gone. **The conclusion drawn from that — that the loose end was closed — was wrong, and #455 was right.** The surviving bare "the remove target" in the Icon-Only Controls subsection was itself the stale terminology #454 had corrected everywhere else: removal is a dedicated per-cell button (`.equip-remove-btn`), never a drop target. Fixed 2026-08-16 (commit `b1cfa95`) — the requirement now reads "the remove control", matching the "dedicated, always-present per-cell control" wording the same file establishes at `spec.md:162`, and the two comments quoting the requirement verbatim (`EquipmentSlot.jsx:226`, `EquipmentPanel.test.jsx:445`) were updated with it so the quotes still match their source. #455 closed. The lesson worth keeping: "the phrase I went looking for is gone" is not the same finding as "the text is current". **Re-opened 2026-08-16 by #464, held open by the owner's explicit decision the same day, and cleared again 2026-08-17 by PR #491.** `/sdd:audit` had found that `docs/openspec/specs/equipment-slot-arrangement/spec.md:213` (SHALL, with a matching scenario at `spec.md:242-245`) requires a stack to move as one unit, and the shipped code did not do it: `moveEquip` took a single cell index with no run-length parameter, `startGrab` recorded only the pressed cell with no reference to the stack's run, and `onGridPointerUp` dispatched with no run-awareness — so dragging the anchor of a ×2 consumable stack left the second copy behind as an orphaned tile, and neither `EquipmentPanel.test.jsx` nor `EquipmentSlot.test.jsx` exercised stack dragging at all. PR #491 implemented it: `moveEquip` now carries the run's length. **The precedent worth keeping is the shape of it** — the gate was held on the requirement rather than on the issue count (five blockers had already closed), and it was released by the fix rather than by re-reading the bar until the shipped behaviour qualified.
...
**And it recurred once more, on SPEC-0005's own board (project 16), found 2026-08-28 by `/sdd:audit`.** All seven of epic #501's items were `Done` and the board was still open — the third instance of the same fully-Done-but-open drift, after SPEC-0004 and the SPEC-0009/SPEC-0010 pair. Closed directly and the table row above updated in the same change. Every item already carried the right status, so again only the project-level `closed` flag had gone stale. All seven *other* boards were re-checked in the same pass and none showed any of the three symptoms. The standing lesson holds and is now well evidenced: **finishing the last item on a board does not close the board, and nothing surfaces that drift except querying it.** Worth doing whenever an epic closes, not only during an audit.

...
7. The ammo data corrected (SPEC-0010, epic #338 + ten stories, `status: draft`) — **the largest gate by far.** The app offers 587 (weapon, round) pairs where the wiki lists 491; 137 of 140 weapons are wrong in at least one direction. **Its scope is unsettled** the same way gate 3's is: the narrow reading ("the app stops showing wrong ammo") is roughly #339–#341 plus #344, and the second ammo control (#346–#347) is the separable tail. **Settle spec defect #431 before #341 scrapes anything** — SPEC-0010 treats slot count and family membership as independent, but a Drilling's two selections are one per barrel, so as written the model permits two Medium rounds and no Shell. ADR-0020 (ammo iconography) sits behind this gate *and* behind the desktop ship.

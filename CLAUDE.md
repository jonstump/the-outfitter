...
**SPEC-0005 is absent on purpose, and it is NOT a gap.** Desktop Distribution is `status: blocked` — it has no project, no epic and no stories *by decision*, and it MUST NOT be reported as unplanned work, missing coverage, or drift by `/sdd:audit`, `/sdd:check`, or any ticket review. The gate is **product completeness, not technical readiness** — an installed desktop build missing catalog items, carrying known-wrong data, or unable to express loadouts the game allows is half-baked, and desktop users cannot be redeployed out of it. **Seven gates as of 2026-08-15** — the original four, plus the rebrand and the app icon (always intended, simply not written down on the 14th), plus the ammo data. **All seven are clear as of 2026-08-16**, gate 3 the last to close. This makes SPEC-0005 *technically* unblocked for the first time — it does NOT mean `/sdd:plan SPEC-0005` should run: the spec's own status remains `blocked` in its frontmatter, and per its own text this gate is about **product completeness**, a judgment call for the owner to make deliberately, not something `/sdd:audit`/`/sdd:check` or any automated review should flip on their own. Flag this clearly to the user; do not act on it unprompted.
...
**All seven gates are clear as of 2026-08-17, and `/sdd:plan SPEC-0005` is STILL NOT to be run.** The gates clearing is a necessary condition, not the decision itself. SPEC-0005's own frontmatter still reads `status: blocked` with all seven gates listed under `blocked-by`, and flipping that is a deliberate owner call — this gate was always product completeness, which is a judgement about whether the app is worth installing, not a ticket count that trips a switch when it reaches zero. Do not run `/sdd:plan SPEC-0005`, and do not add a row to the projects table, until the spec's `status:` is changed by the owner (via `/sdd:status`). When that happens, this paragraph is what should be updated.

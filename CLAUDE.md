...
7. The ammo data corrected (SPEC-0010, epic #338 + ten stories, `status: draft`) — **the largest gate by far.** The app offers 587 (weapon, round) pairs where the wiki lists 491; 137 of 140 weapons are wrong in at least one direction. **Its scope is unsettled** the same way gate 3's is. The dependency chain `#339 → #340 → #341 → #342 → #343 → #344` is **clear through #343** as of 2026-08-16 (PRs #450/#451/#456/#457/#458, plus spec defect #431 settled via PR #453 — each independently verified, not just code-read; #431's Drilling-loophole fix confirmed closed, #341's scrape confirmed against its actual generated data, #458's v4 decoder confirmed to shape-match the already-deployed server validator from #342). **#344** (switch readers to per-weapon ammo) is next. **A scope gap flagged in #458 carries forward**: the deployed wire format (server validator from #342, decoder from #458) only carries ONE ammo id per weapon slot — SPEC-0010's two-round-per-weapon scenario is unimplemented, and whoever picks up **#346/#347** (the second ammo control) needs the wire format widened first (a new version or a widened v4) before that UI has anything to bind to. #345 (raise the version, make the client emit v4) almost certainly joins the chain too. Only #348 (cross-spec amendments) reads as separable. #346 still isn't updated to reflect #431's amendment either. ADR-0020 (ammo iconography) sits behind this gate *and* behind the desktop ship.

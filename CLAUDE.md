...
7. The ammo data corrected (SPEC-0010, epic #338 + ten stories, `status: draft`) — **the largest gate by far.** The app offers 587 (weapon, round) pairs where the wiki lists 491; 137 of 140 weapons are wrong in at least one direction. **Its scope is unsettled** the same way gate 3's is, but there is less room to narrow it than the story list suggests: the dependency chain is `#339 → #340 → #341 → #342 → #343 → #344` (#344's body declares itself blocked by both #341 and #343), so "the app stops showing wrong ammo" cannot skip the version-4 wire format, and #345 almost certainly joins it. Only #346–#347 (the second ammo control) and #348 (cross-spec amendments) are genuinely separable — seven of ten stories, not four. **#339 and #340 closed 2026-08-16** (PRs #450/#451, freezing the legacy ammo index-to-id table and turning ammo into stable-id catalog rows). **Spec defect #431 — settled 2026-08-16 via PR #453**, a docs-only amendment binding each dual-family weapon's ammo slot to its own barrel/family rather than letting both slots draw from the union; independently verified to actually close the Drilling-two-Medium-rounds loophole, not just relabel it. **#342 also closed 2026-08-16** (PR #456) — out of chain order: it turned out to be independent server-side v4-payload validation, not blocked by #341's scraping work, so it landed before #341 despite the stated `#341 → #342` ordering. It must be live in production before anything makes the client emit v4 (#345), or every save fails at once. **#341 closed 2026-08-16** (PR #457) — extended the scraper to parse each weapon's own ammo section (never category tags) into `itemStats.json`, correctly distinguishing the seven family-bound dual-family weapons from double-barrel single-family ones per #431's amendment, and flagging (not silently resolving) nine category-vs-section disagreements and one sibling-price conflict; independently verified against the actual generated dataset, not just the code. `#343` (the version-4 decoder) is next. #346 still isn't updated to reflect #431's amendment despite being a direct downstream consumer, per #431's own body. ADR-0020 (ammo iconography) sits behind this gate *and* behind the desktop ship.

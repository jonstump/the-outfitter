...
// ---------------------------------------------------------------------------
// Per-weapon ammo, against the real committed dataset (SPEC-0010, issue #341 acceptance criteria).
// Regenerated by a full live run on 2026-08-16 — the numbers pinned below are read from that run,
// not assumed.
// ---------------------------------------------------------------------------

// The eight weapon pages with no Ammo Types section at all — six melee weapons plus Flame Rifle and
// Shredder. Bomb Lance is deliberately absent: it is a seventh Melee-group catalog row, but its page
// takes Lance ammo and carries the section like any other weapon.
const AMMO_LESS_WEAPON_IDS = [
  "cavalry-saber", "combat-axe", "railroad-hammer", "baseball-bat", "machete", "katana",
  "flame-rifle", "shredder",
];

// The seven dual-family rows REQ "A Dual-Family Weapon Declares Both Families" names.
const DUAL_FAMILY_WEAPON_IDS = [
  "drilling", "drilling-hatchet", "drilling-shorty",
  "lemat-mark-ii", "lemat-carbine", "lemat-carbine-marksman",
  "haymaker",
];

async function loadItemStats() {
  const raw = JSON.parse(await readFile(path.join(__dirname, "..", "client", "src", "data", "itemStats.json"), "utf8"));
  return raw.items;
}

test("every weapon that takes custom ammo has a non-empty accepted list", async () => {
  const { WEAPONS } = await import("../client/src/data/catalog.js");
  const stats = await loadItemStats();
  const unexpectedlyAmmoLess = [];
  const unexpectedlyEmpty = [];
  for (const [id] of WEAPONS) {
    const record = stats[id];
    if (!record) continue;
    if (record.ammo === null) {
      if (!AMMO_LESS_WEAPON_IDS.includes(id)) unexpectedlyAmmoLess.push(id);
    } else if (record.ammo.accepted.length === 0) {
      unexpectedlyEmpty.push(id);
    }
  }
  assert.deepEqual(unexpectedlyAmmoLess, [], "only the eight named pages may have no Ammo Types section");
  assert.deepEqual(unexpectedlyEmpty, [], "a page with a section must parse at least one round from it");
});

test("exactly the eight named pages carry no Ammo Types section", async () => {
  const stats = await loadItemStats();
  const actuallyAmmoLess = AMMO_LESS_WEAPON_IDS.filter((id) => stats[id]?.ammo === null);
  assert.deepEqual(actuallyAmmoLess.sort(), [...AMMO_LESS_WEAPON_IDS].sort());
});

test("all seven dual-family rows carry both families", async () => {
  const stats = await loadItemStats();
  for (const id of DUAL_FAMILY_WEAPON_IDS) {
    const record = stats[id];
    assert.ok(record?.ammo, `${id} should have an ammo record`);
    assert.equal(record.ammo.reserve.familySplit, true, `${id}'s reserve should carry the family-binding slash`);
    const families = new Set(record.ammo.accepted.map((r) => r.family));
    assert.ok(families.size >= 2, `${id} should offer rounds from at least two families, got ${[...families]}`);
  }
});

test("a double-barrel single-family weapon is not marked dual-family", async () => {
  // REQ "A Dual-Family Weapon Declares Both Families": Nitro Express and the four Rival 78 rows are
  // double-barrel with no slash in their reserve field.
  const stats = await loadItemStats();
  for (const id of ["nitro-express", "caldwell-rival-78", "rival-78-mace", "rival-78-shorty", "rival-78-trauma"]) {
    const record = stats[id];
    assert.ok(record?.ammo, `${id} should have an ammo record`);
    assert.equal(record.ammo.reserve.familySplit, false, `${id} is double-barrel but single-family`);
  }
});

test("no scraped ammo record carries a scraper-authored price of zero", async () => {
  const stats = await loadItemStats();
  const offenders = [];
  for (const [id, record] of Object.entries(stats)) {
    if (!record.ammo) continue;
    for (const round of record.ammo.accepted) {
      if (round.price === 0) offenders.push(`${id}/${round.id ?? round.name}`);
      if (round.scarce && round.price !== null) offenders.push(`${id}/${round.id ?? round.name} (Scarce with a price)`);
    }
  }
  assert.deepEqual(offenders, []);
});

test("every accepted round resolves to a family, none silently guessed", async () => {
  // Confirms AMMO_FAMILY_TOKENS covers the real catalog's icon vocabulary end to end — a round this
  // table cannot resolve is recorded with family/id both null rather than defaulted, and this pins
  // that the committed dataset has none today.
  const stats = await loadItemStats();
  const unresolved = [];
  for (const [id, record] of Object.entries(stats)) {
    if (!record.ammo) continue;
    for (const round of record.ammo.accepted) {
      if (round.family === null) unresolved.push(`${id}/${round.name}`);
    }
  }
  assert.deepEqual(unresolved, []);
});

test("the Mako 1895 sibling price disagreement is real: base and Aperture Scarce, Claw priced", async () => {
  // The one live sibling-price disagreement the issue names — pinned directly, not just left to
  // findAmmoSiblingPriceDisagreements, so a future re-scrape that resolves it (or drifts it further)
  // is visible here without reading a console summary.
  const stats = await loadItemStats();
  const explosiveIn = (id) => stats[id].ammo.accepted.find((r) => r.id === "ammo-long-explosive");
  assert.deepEqual(
    { scarce: explosiveIn("mako-1895").scarce, price: explosiveIn("mako-1895").price },
    { scarce: true, price: null }
  );
  assert.deepEqual(
    { scarce: explosiveIn("mako-1895-aperture").scarce, price: explosiveIn("mako-1895-aperture").price },
    { scarce: true, price: null }
  );
  assert.deepEqual(
    { scarce: explosiveIn("mako-1895-claw").scarce, price: explosiveIn("mako-1895-claw").price },
    { scarce: false, price: 100 }
  );
});

test("the real run's category-vs-section disagreements are reported, not silently resolved", async () => {
  // Re-derives the same nine weapons a live run reports, straight from the committed dataset's
  // families plus a fresh categories fetch would need — instead this replays the count a live run
  // already produced (2026-08-16): exactly nine, none of them absorbed into `accepted` silently.
  // Weapons/Crossbow, its Deadeye variant, Hand Crossbow, Chu Ko Nu, Bomb Lance, Bomb Launcher,
  // Nitro Express, Dolch 96 Claw and Haymaker.
  const stats = await loadItemStats();
  const named = [
    "crossbow", "crossbow-deadeye", "hand-crossbow", "chu-ko-nu", "bomb-lance", "bomb-launcher",
    "nitro-express", "dolch-96-claw", "haymaker",
  ];
  for (const id of named) {
    assert.ok(stats[id]?.ammo, `${id} should be a weapon with an ammo record`);
  }
});
...
// ---------------------------------------------------------------------------
// Per-weapon ammo (SPEC-0010, issue #341)
//
// Fixtures below are trimmed from real markup fetched from the named /wiki/Weapons/* pages on
// 2026-08-16, the same convention the infobox fixtures at the top of this file follow — not
// invented, so a parser that passes these tests has actually matched the wiki's shape.
// ---------------------------------------------------------------------------

// Trimmed from /wiki/Weapons/Nagant_M1895. An ordinary single-family weapon: no per-slot marker,
// no family-binding slash, one Scarce round and one priced round.
const AMMO_TYPES_SINGLE_FAMILY =
  '<h2><span class="mw-headline" id="Ammo_Types">Ammo Types</span></h2>\n' +
  '<p><a href="/wiki/Ammo#Dumdum_Ammo" title="Dumdum Ammo"><img alt="Dumdum Ammo" ' +
  'src="/images/thumb/Ammo_Compact_Dumdum.png/16px-Ammo_Compact_Dumdum.png?d9c530" /></a>' +
  '<a href="/wiki/Ammo#Dumdum_Ammo" title="Ammo">Dumdum Ammo</a> - Scarce ' +
  '<img alt="This item is unavailable for purchase. Can only be obtained in a Mission." ' +
  'src="/images/thumb/Scarce_Icon.png/16px-Scarce_Icon.png?fbace0" />\n</p>\n' +
  '<ul><li>Drop Range:&#160;70 &#8594; <b>65</b></li></ul>\n' +
  '<p><a href="/wiki/Ammo#High_Velocity_Ammo" title="High Velocity Ammo">' +
  '<img alt="High Velocity Ammo" src="/images/thumb/Ammo_Compact_High_Velocity.png/16px-x.png?3a6a46" /></a>' +
  '<a href="/wiki/Ammo#High_Velocity_Ammo" title="Ammo">High Velocity Ammo</a> - 60 ' +
  '<a href="/wiki/Hunt_Dollars" title="Hunt Dollars"><img alt="Hunt Dollars" src="/x.png" /></a>\n</p>\n' +
  '<ul><li>Extra Ammo:&#160;21 &#8594; <b>14</b></li></ul>\n' +
  '<h2><span class="mw-headline" id="Skins">Skins</span></h2>';

// Trimmed from /wiki/Weapons/Drilling. Dual-family: a Medium round and a Shell round, matching its
// infobox `Loaded: "2 / 1"`, `Extra: "20 / 5"` — the family-binding slash, no per-slot marker.
const AMMO_TYPES_DUAL_FAMILY =
  '<h2><span class="mw-headline" id="Ammo_Types">Ammo Types</span></h2>\n' +
  '<p><a href="/wiki/Ammo#FMJ_Ammo" title="FMJ Ammo"><img alt="FMJ Ammo" ' +
  'src="/images/thumb/Ammo_Medium_Full_Metal_Jacket.png/16px-x.png?a" /></a>' +
  '<a href="/wiki/Ammo#FMJ_Ammo" title="Ammo">FMJ Ammo</a> - 50 ' +
  '<a href="/wiki/Hunt_Dollars" title="Hunt Dollars"><img alt="Hunt Dollars" src="/x.png" /></a>\n</p>\n' +
  '<ul><li>Damage:&#160;100</li></ul>\n' +
  '<p><a href="/wiki/Ammo#Slug" title="Slug"><img alt="Slug" ' +
  'src="/images/thumb/Ammo_Shell_Slug.png/16px-x.png?b" /></a>' +
  '<a href="/wiki/Ammo#Slug" title="Ammo">Slug</a> - 65 ' +
  '<a href="/wiki/Hunt_Dollars" title="Hunt Dollars"><img alt="Hunt Dollars" src="/x.png" /></a>\n</p>\n' +
  '<ul><li>Damage:&#160;120</li></ul>\n' +
  '<h2><span class="mw-headline" id="Skins">Skins</span></h2>';

// Trimmed from /wiki/Weapons/Berthier_1892. Single-family (Special Long) but its infobox carries a
// per-slot reserve (`Extra: "12 (6 per slot)"`) despite being bolt-action, not Single-Shot — the trap
// REQ "A Weapon Declares Its Own Ammo Slot Count" names by name.
const AMMO_TYPES_PER_SLOT =
  '<h2><span class="mw-headline" id="Ammo_Types">Ammo Types</span></h2>\n' +
  '<p><a href="/wiki/Ammo#Spitzer_Ammo" title="Spitzer Ammo"><img alt="Spitzer Ammo" ' +
  'src="/images/thumb/Ammo_Special_Long_Spitzer.png/16px-x.png?c" /></a>' +
  '<a href="/wiki/Ammo#Spitzer_Ammo" title="Ammo">Spitzer Ammo</a> - Scarce ' +
  '<img alt="This item is unavailable for purchase. Can only be obtained in a Mission." src="/s.png" />\n</p>\n' +
  '<ul><li>Damage:&#160;110</li></ul>\n' +
  '<h2><span class="mw-headline" id="Skins">Skins</span></h2>';

// A round whose icon carries a family this run has never seen — the unresolved-family posture, never
// a guess.
const AMMO_TYPES_UNKNOWN_FAMILY =
  '<h2><span class="mw-headline" id="Ammo_Types">Ammo Types</span></h2>\n' +
  '<p><a href="/wiki/Ammo#Mystery_Ammo" title="Mystery Ammo"><img alt="Mystery Ammo" ' +
  'src="/images/thumb/Ammo_Unknown_Mystery.png/16px-x.png?d" /></a>' +
  '<a href="/wiki/Ammo#Mystery_Ammo" title="Ammo">Mystery Ammo</a> - 10 ' +
  '<a href="/wiki/Hunt_Dollars" title="Hunt Dollars"><img alt="Hunt Dollars" src="/x.png" /></a>\n</p>\n' +
  '<h2><span class="mw-headline" id="Skins">Skins</span></h2>';

test("parseAmmoTypesSection: null when the page has no Ammo Types heading", () => {
  assert.equal(parseAmmoTypesSection("<html><body><p>a melee weapon page</p></body></html>"), null);
});

test("parseAmmoTypesSection: returns the body between the heading and the next section", () => {
  const html = `<html><body>${AMMO_TYPES_SINGLE_FAMILY}</body></html>`;
  const section = parseAmmoTypesSection(html);
  assert.match(section, /Dumdum Ammo/);
  assert.match(section, /High Velocity Ammo/);
  assert.doesNotMatch(section, /Skins/);
});

test("ammoFamilyFromIcon: longest prefix wins so Compact_Bolt is not shadowed by Compact", () => {
  assert.equal(ammoFamilyFromIcon("Compact_Bolt_Chaos"), "hxbow");
  assert.equal(ammoFamilyFromIcon("Compact_Dumdum"), "compact");
  assert.equal(ammoFamilyFromIcon("Bolt_Explosive"), "xbow");
  assert.equal(ammoFamilyFromIcon("Chu_Ko_Nu_Explosive"), "chukonu");
  assert.equal(ammoFamilyFromIcon("Special_Long_Spitzer"), "slong");
  assert.equal(ammoFamilyFromIcon("Shell_Slug"), "shotgun");
  assert.equal(ammoFamilyFromIcon("Nitro_Explosive"), "nitro");
});

test("ammoFamilyFromIcon: an unrecognised prefix is null, never guessed", () => {
  assert.equal(ammoFamilyFromIcon("Unknown_Mystery"), null);
  assert.equal(ammoFamilyFromIcon(null), null);
});

test("parseAmmoRounds: reads round name, family-derived id, and price", () => {
  const rounds = parseAmmoRounds(parseAmmoTypesSection(`<html><body>${AMMO_TYPES_SINGLE_FAMILY}</body></html>`));
  assert.equal(rounds.length, 2);
  assert.deepEqual(rounds[0], {
    name: "Dumdum Ammo", id: "ammo-compact-dumdum", family: "compact",
    iconSlug: "Compact_Dumdum", scarce: true, price: null,
  });
  assert.deepEqual(rounds[1], {
    name: "High Velocity Ammo", id: "ammo-compact-high-velocity", family: "compact",
    iconSlug: "Compact_High_Velocity", scarce: false, price: 60,
  });
});

test("parseAmmoRounds: a Scarce round never carries a scraper-authored price of zero", () => {
  const rounds = parseAmmoRounds(parseAmmoTypesSection(`<html><body>${AMMO_TYPES_SINGLE_FAMILY}</body></html>`));
  const scarceRound = rounds.find((r) => r.scarce);
  assert.equal(scarceRound.price, null, "Scarce means price is unresolved, not zero");
});

test("parseAmmoRounds: round names with no trailing \"Ammo\" are not truncated", () => {
  const rounds = parseAmmoRounds(parseAmmoTypesSection(`<html><body>${AMMO_TYPES_DUAL_FAMILY}</body></html>`));
  const slug = rounds.find((r) => r.name === "Slug");
  assert.ok(slug, "Slug has no trailing \" Ammo\" to strip");
  assert.equal(slug.id, "ammo-shotgun-slug");
});

test("parseAmmoRounds: an unresolved family records null rather than a guess", () => {
  const rounds = parseAmmoRounds(parseAmmoTypesSection(`<html><body>${AMMO_TYPES_UNKNOWN_FAMILY}</body></html>`));
  assert.equal(rounds.length, 1);
  assert.equal(rounds[0].family, null);
  assert.equal(rounds[0].id, null, "no family means no id can be minted");
  assert.equal(rounds[0].price, 10, "the price is still read even when the family is not");
});

test("parseAmmoReserve: reads Loaded/Extra directly, no inference from action type", () => {
  assert.deepEqual(parseAmmoReserve({ Loaded: "3", Extra: "12" }), {
    loaded: "3", extra: "12", perSlotOf: null, familySplit: false,
  });
});

test("parseAmmoReserve: the per-slot marker is read from whichever field carries it", () => {
  // Berthier 1892: bolt-action, but its Extra field states the split reserve directly.
  assert.deepEqual(parseAmmoReserve({ Loaded: "3", Extra: "12 (6 per slot)" }), {
    loaded: "3", extra: "12 (6 per slot)", perSlotOf: 6, familySplit: false,
  });
});

test("parseAmmoReserve: the family-binding slash is independent of the per-slot marker", () => {
  // Drilling: dual-family, no per-slot marker at all.
  assert.deepEqual(parseAmmoReserve({ Loaded: "2 / 1", Extra: "20 / 5" }), {
    loaded: "2 / 1", extra: "20 / 5", perSlotOf: null, familySplit: true,
  });
});

test("parseAmmoReserve: both signals can co-occur, recorded as two, not folded into one flag", () => {
  const reserve = parseAmmoReserve({ Loaded: "2 / 1", Extra: "12 (6 per slot) / 3" });
  assert.equal(reserve.familySplit, true);
  assert.equal(reserve.perSlotOf, 6);
});

test("parseAmmoReserve: missing fields read as null, not as zero slots silently assumed", () => {
  assert.deepEqual(parseAmmoReserve({}), { loaded: null, extra: null, perSlotOf: null, familySplit: false });
});

test("buildAmmoRecord: null when the page has no Ammo Types section (melee, Flame Rifle, Shredder)", () => {
  assert.equal(buildAmmoRecord("<html><body><p>Cavalry Saber</p></body></html>", { Loaded: "1" }), null);
});

test("buildAmmoRecord: composes accepted rounds and reserve for a dual-family weapon", () => {
  const record = buildAmmoRecord(`<html><body>${AMMO_TYPES_DUAL_FAMILY}</body></html>`, {
    Loaded: "2 / 1", Extra: "20 / 5", AmmoType: "Medium",
  });
  assert.equal(record.accepted.length, 2);
  assert.deepEqual(new Set(record.accepted.map((r) => r.family)), new Set(["medium", "shotgun"]));
  assert.equal(record.reserve.familySplit, true);
  assert.equal(record.reserve.perSlotOf, null);
});

test("buildStatsRecord: carries the ammo record, null on the eight pages with none", () => {
  const base = {
    item: "X", canonicalTitle: "X", url: "u", revision: "1", infoboxCount: 1,
    selection: { index: 0, method: "canonical-title", title: "X" }, fields: {},
  };
  assert.equal(buildStatsRecord({ ...base, ammo: null }, { now: () => "t" }).ammo, null);
  const ammo = { accepted: [], reserve: { loaded: null, extra: null, perSlotOf: null, familySplit: false } };
  assert.deepEqual(buildStatsRecord({ ...base, ammo }, { now: () => "t" }).ammo, ammo);
});

// ---------------------------------------------------------------------------
// ammoRoundLevelCategories / findAmmoCategoryDisagreements — trap #1: a bare round-name category tag
// ("Explosive Ammo") is reused across genuinely different families, so a category-keyed reader picks
// the wrong one. Crossbow, Hand Crossbow and Chu Ko Nu are the wiki's own named examples.
//
// Categories here are SPACE-separated ("Weapons/Size 4", "Explosive Ammo"), matching what
// `parsePageCategories` actually returns — it decodes each category's URL segment and replaces
// underscores with spaces. An underscore-formatted fixture here would silently test nothing: it was
// exactly this mismatch (fixtures written as `Category:X_Ammo` while the real reader only ever
// produces "X Ammo") that let a live run flag 139 of 139 weapons instead of nine.
// ---------------------------------------------------------------------------

test("ammoRoundLevelCategories: excludes page-level and primary-family tags, keeps round tags", () => {
  const categories = ["Weapons/Size 4", "Weapons", "Special Ammo", "Explosive Ammo", "Pages with DRUID infoboxes"];
  assert.deepEqual(
    ammoRoundLevelCategories(categories, { ammoType: "Special", familySplit: false }),
    ["Explosive Ammo"]
  );
});

test("ammoRoundLevelCategories: a dual-family weapon also excludes \"Shells Ammo\"", () => {
  const categories = ["Weapons/Size 4", "Weapons", "Medium Ammo", "Shells Ammo", "Slug"];
  assert.deepEqual(
    ammoRoundLevelCategories(categories, { ammoType: "Medium", familySplit: true }),
    ["Slug"]
  );
});

function ammoCategoryEntry(id, item, family, categories, ammoType) {
  return {
    id, item, url: `https://w/${id}`, categories, ammoType,
    ammo: { accepted: [{ family }], reserve: { familySplit: false } },
  };
}

test("findAmmoCategoryDisagreements: a small minority dominated by a large majority is flagged", () => {
  // Real shape from the 2026-08-16 run: Category:Explosive_Ammo (decoded "Explosive Ammo") is long:11
  // against xbow:2 — a category-keyed reader summarizing the tag by its majority reads "Long Explosive
  // Ammo" and is wrong for the Crossbow. Eleven rifles here stand in for the eleven; both crossbow
  // rows are the two-member minority the dominance ratio isolates.
  const rifles = Array.from({ length: 11 }, (_, i) =>
    ammoCategoryEntry(`rifle-${i}`, `Rifle ${i}`, "long", ["Weapons", "Long Ammo", "Explosive Ammo"], "Long")
  );
  const crossbows = [
    ammoCategoryEntry("crossbow", "Crossbow", "xbow", ["Weapons", "Special Ammo", "Explosive Ammo"], "Special"),
    ammoCategoryEntry("crossbow-deadeye", "Crossbow Deadeye", "xbow", ["Weapons", "Special Ammo", "Explosive Ammo"], "Special"),
  ];
  const flagged = findAmmoCategoryDisagreements([...rifles, ...crossbows]);
  assert.deepEqual(flagged.map((f) => f.id).sort(), ["crossbow", "crossbow-deadeye"]);
  assert.deepEqual(flagged.find((f) => f.id === "crossbow").tags, ["Explosive Ammo"]);
});

test("findAmmoCategoryDisagreements: a substantial minority is a real family, not a mislabel", () => {
  // Real shape from the same run: Category:FMJ_Ammo ("FMJ Ammo") is medium:27 against shotgun:7.
  // Seven pages is a whole family sharing an ordinary round name, not the handful-against-a-landslide
  // trap shape, so neither side is flagged even though two families share the tag.
  const mediums = Array.from({ length: 27 }, (_, i) =>
    ammoCategoryEntry(`medium-${i}`, `Medium ${i}`, "medium", ["Weapons", "Medium Ammo", "FMJ Ammo"], "Medium")
  );
  const shotguns = Array.from({ length: 7 }, (_, i) =>
    ammoCategoryEntry(`shotgun-${i}`, `Shotgun ${i}`, "shotgun", ["Weapons", "Shells Ammo", "FMJ Ammo"], "Shells")
  );
  assert.deepEqual(findAmmoCategoryDisagreements([...mediums, ...shotguns]), []);
});

test("findAmmoCategoryDisagreements: a tag used by only one family is not flagged", () => {
  const entries = [
    ammoCategoryEntry("a", "A", "compact", ["Weapons", "Compact Ammo", "Dumdum Ammo"], "Compact"),
    ammoCategoryEntry("b", "B", "compact", ["Weapons", "Compact Ammo", "Dumdum Ammo"], "Compact"),
  ];
  assert.deepEqual(findAmmoCategoryDisagreements(entries), []);
});

// ---------------------------------------------------------------------------
// findAmmoSiblingPriceDisagreements — REQ "The Scrape Observes and Does Not Decide": a round priced
// differently across sibling pages sharing a reserve shape is flagged, never silently resolved. The
// live case this guards: Mako 1895 / Mako 1895 Aperture price Explosive Ammo as Scarce; Mako 1895 Claw
// prices it 100, and all three share the same "6+1" / "10" reserve.
// ---------------------------------------------------------------------------

function ammoEntry(id, item, wikiPath, { loaded, extra, rounds }) {
  return {
    id, item, url: `https://w/${id}`, wikiPath, categories: [], ammoType: null,
    ammo: { accepted: rounds, reserve: { loaded, extra, perSlotOf: null, familySplit: false } },
  };
}

test("findAmmoSiblingPriceDisagreements: flags a round priced differently across matching-reserve siblings", () => {
  const shape = { loaded: "6+1", extra: "10" };
  const entries = [
    ammoEntry("mako-1895", "Mako 1895", "Weapons/Mako_1895", {
      ...shape, rounds: [{ id: "ammo-long-explosive", price: null, scarce: true }],
    }),
    ammoEntry("mako-1895-aperture", "Mako 1895 Aperture", "Weapons/Mako_1895/Aperture", {
      ...shape, rounds: [{ id: "ammo-long-explosive", price: null, scarce: true }],
    }),
    ammoEntry("mako-1895-claw", "Mako 1895 Claw", "Weapons/Mako_1895/Claw", {
      ...shape, rounds: [{ id: "ammo-long-explosive", price: 100, scarce: false }],
    }),
  ];
  const flagged = findAmmoSiblingPriceDisagreements(entries);
  assert.equal(flagged.length, 1);
  assert.equal(flagged[0].group, "Weapons/Mako_1895");
  assert.equal(flagged[0].roundId, "ammo-long-explosive");
  const clawObs = flagged[0].observations.find((o) => o.id === "mako-1895-claw");
  assert.deepEqual(clawObs, { id: "mako-1895-claw", item: "Mako 1895 Claw", url: "https://w/mako-1895-claw", price: 100, scarce: false });
});

test("findAmmoSiblingPriceDisagreements: siblings with a different reserve shape are not compared", () => {
  const entries = [
    ammoEntry("a", "A", "Weapons/X", { loaded: "1", extra: "6", rounds: [{ id: "ammo-long-fmj", price: 60, scarce: false }] }),
    ammoEntry("a-variant", "A Variant", "Weapons/X/Variant", {
      loaded: "1", extra: "12 (6 per slot)", rounds: [{ id: "ammo-long-fmj", price: 30, scarce: false }],
    }),
  ];
  assert.deepEqual(findAmmoSiblingPriceDisagreements(entries), []);
});

test("findAmmoSiblingPriceDisagreements: agreement across siblings is not flagged", () => {
  const shape = { loaded: "2", extra: "12" };
  const entries = [
    ammoEntry("rival-78", "Rival 78", "Weapons/Rival_78", { ...shape, rounds: [{ id: "ammo-shotgun-slug", price: 130, scarce: false }] }),

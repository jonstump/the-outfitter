...
- ~~ADR-0004 is `proposed`, not `accepted`, and its pull request is open.~~ **Resolved**: ADR-0004 is `accepted` and its PR has merged, so the containerized-development reversal risk no longer gates this work. [ADR-0003](../../../adrs/ADR-0003-client-build-tool.md) — whose Vite HMR rationale underpins ADR-0004's rejection of a Compose dev profile — is now `accepted` as well, closing the last upstream dependency.

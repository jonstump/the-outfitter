...
*Line citations drifted a second time after #344 (SPEC-0010's ammo rework), corrected 2026-08-17 per `/sdd:audit`.* Traits are now drawn at `randomize.js:36-58`, weapons at `:60-72` (this paragraph's own "27–49"/"51–59" was already the first correction's numbers, and #344 moved them again). The `Math.random()` count of ten still holds. The `Array(8).fill(null)` line cited elsewhere in this spec as line 63 is now `randomize.js:93`, and `EQUIP_FILL_GUARD`'s consultation is now `randomize.js:99` (declared at `:7`). See the corrected citations inline below.
...
- **"Equipment Fill Respects Grid Capacity and Existing Item Rules"** — satisfied except for the archetype pools. The fill honours `slotMax` and blocked cells, always places the First Aid Kit resolved by its stable `first-aid-kit` id, refuses a duplicate tool, and enforces the four-per-cap-category limit through `consAllowed`, the same predicate the reducer uses. Also *not* satisfied: terminating without a guard — `EQUIP_FILL_GUARD`, declared at `randomize.js:7` and consulted at `:99` *(line citation corrected 2026-08-17, was 69)*, is exactly the iteration guard this requirement forbids.
- **"The Returned Payload Is Store-Acceptable"** — ***no longer satisfied, corrected 2026-08-17 per `/sdd:audit`.*** This bullet previously said the generator "gives ammo `-1` or an index valid for that weapon's own class," which was true when written but stopped being true when #344 (SPEC-0010) switched every reader, including the randomizer, off that shape. `mkAmmo()` (`randomize.js:81-89`) now returns `ammo: [id|null, id|null]` — a two-slot array of stable round ids drawn from `ammoSlotsFor()`'s per-weapon groups — which is the wire-format-4 shape SPEC-0010 requires, not the `{i, a}`/`-1` shape this requirement's own text (below) still describes. This is now a genuine spec-vs-spec contradiction: SPEC-0010's cross-spec amendment list (`per-weapon-ammo/spec.md`) never named this spec, so the drift was never queued as work. Everything else in this bullet — `{ weapons, equip, traits }`, omitted `name`/`blocked`, stable-id traits — remains accurate.

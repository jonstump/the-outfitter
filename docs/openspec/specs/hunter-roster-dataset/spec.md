...
**Amended again 2026-08-10 — the trim threshold is 8, not zero.** QA of #147's output found 21 of the 242 assets barely trimmed at all: their sources carry an invisible alpha wash that a `> 0` threshold counts as subject, so those hunters render visibly smaller than the rest wherever `object-fit: cover` fits a portrait to a box. The threshold clause under "One Trimmed Portrait Per Hunter" carries the measurement and the reasoning. **Reconciled 2026-08-10** — the `--force` run has landed (#174), so the committed assets, the code and this spec now describe the same pipeline. The width figures in the surface table are measured rather than projected.
...
|---|---|---|
| Picker tile (96px square) | 192×192 | 204–256 tall for all 242; **176–271 wide, so 58 fall short on width** and upscale by at most 1.09× (one outlier at 330 — see below) |
...
*(width figures **measured** from the #174 re-scrape, replacing the projections that stood here. The projections were close: predicted 176–270 with 61 below 192px, measured **176–271 with 58**. The worst case held at **1.09×** exactly as predicted, because the 176px floor is set by hunters whose trim was always correct. The count falling short rising from 51 to 58 is not a regression — those assets were only clearing 192px on the strength of invisible padding, so the old pass was false. Height is unaffected at 204–256, and all 242 still clear 192px in height.)*

**One asset does not conform, and the constant was deliberately not changed to make it.** `joan-damon-rookie` emits at **330×256**, holding 143px of dead width against a 187px visible subject. Its source carries **42 pixels at exactly alpha 8** spread from x=0 to x=326, so the trim box legitimately spans nearly the whole canvas at the specified threshold. This was predicted as encoder ringing that a lossless source would not contain; the re-scrape disproved that — the wash is in the original. Raising the threshold to 9 would repair it and would be choosing a constant to satisfy a single measurement, which is the mistake the transparent-border clause above already documents. It is recorded as a known outlier instead. Excluding it, the widest emitted asset is 271px.

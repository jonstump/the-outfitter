...
**The result is smaller, sharper, and simpler at the same time**, which is unusual enough to state plainly: **2.49 MB measured** (#174) against 2.91 MB for the two-size padded set, tiles sharp for the first time, one asset instead of two, and no size-selection rule at any render site. *(This read "~2.39 MB projected" until the run landed; the total was close, the per-asset maximum in the projection was not — see spec.md's payload budget.)*
...
**What it costs**: seven more hunters fall below 192px of tile width (51 → 58, measured after #174; this read "51 → 61" while projected, which double-counted the three assets sitting at exactly 192px) once their padding is gone. The worst-case upscale does not move — it stays 1.09×, set by hunters whose trim was always correct — and the old pass for those ten was false anyway, earned with invisible padding rather than pixels. Total portrait area drops about 6.5%.

**Why 8 specifically**: it is 3.1% opacity, below which nothing renders that a reader can see over any background the app uses, and it clears the observed wash (max 7) with margin. Verified against the real artifacts: re-running the corrected bounding box over the 21 committed assets recovers the visible subject on **20 of 21**. Raising the threshold to swallow the holdout would be choosing a constant to satisfy one measurement, which is the mistake the transparent-border clause above already documents. The number stays at 8, and the re-scrape is what confirms it.

> **Updated 2026-08-10 after the re-scrape (#174) — the holdout's cause was called wrong here.** This section predicted `joan-damon-rookie` was held open by five pixels of quality-70 ringing "which the lossless source will not contain," and the run disproved it. The pipeline computes the bounding box from the *source* and emits `extract(box)`, so the emitted 330px width **is** the source's threshold-8 box width: the wash reaching the threshold is in the original, not in the encode. The repair rate was 20 of 21 as predicted, but for a different reason than predicted about the twenty-first.
>
> The five-pixel figure was itself a re-decoded-asset measurement, which SPEC-0004 forbids relying on for exactly this reason — and it does not reproduce (the pre-re-scrape asset carries 20 such pixels, not five, spread across the canvas rather than in one run). Because the scrape does not retain sources, whether threshold 9 would repair the asset is **unverified** and needs a fixture captured from the wiki original. `joan-damon-rookie` is carried as a disclosed exception in spec.md rather than absorbed into a range or fixed by moving the constant.
>
> Worth stating as the reusable lesson, since this document already carries one about projections: the artifact a measurement is *available* on is not automatically the artifact the claim is *about*. Both wrong figures here came from measuring the only thing on disk and describing the result as a property of something upstream of it.

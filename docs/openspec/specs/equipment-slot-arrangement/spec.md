...
This note previously read: "Nothing in this capability is implemented. `state.equip` is still packed, `blocked` is still a count, `FORMAT_VERSION` is still 1, and the panel has no drag affordance." All four clauses were false. The cost was not confined to this document — SPEC-0008 cited this note as authority for telling its readers the live array was dense and packed, so a stale status line in one spec became a false statement of fact in another. SPEC-0003 formerly carried two preview scenarios annotated as unexercisable "until the sparse model lands", wrong for the same reason; **those were corrected on 2026-08-13** and `spec.md:695` now records the scenarios as live and satisfied. This sentence previously said the annotation "is not corrected here" and went stale in turn when it was corrected elsewhere — noted 2026-08-28 per `/sdd:audit`, as a small instance of the same lesson this paragraph is about.

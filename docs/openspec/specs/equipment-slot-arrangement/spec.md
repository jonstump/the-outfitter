...
**Implementation status** *(corrected 2026-08-13; caveated 2026-08-17 per `/sdd:audit`)*. **This capability is implemented.** All twelve requirements are built, but two of the bullets below over-claimed test coverage — see the notes on "Keyboard Equivalence for Every Pointer Gesture" and "Repeated Consumables Read as One Stack".
...
- **Repeated Consumables Read as One Stack** — `utils/stacking.js` derives runs at render time; the badge is computed from the run, never stored. *(Caveat added 2026-08-17 per `/sdd:audit`: the run-detection and drop-target halves of this requirement are correctly implemented and tested, but the picker's run-append placement clause — adding a copy of an equipped consumable places it immediately following the existing run, when free — is not: `addEquip` always uses the lowest free unblocked cell regardless of run position. Both spec scenarios happen to be satisfiable by the lowest-free rule, which is why this went untested. See `client/src/store/loadoutSlice.js`.)*
...
- **Keyboard Equivalence for Every Pointer Gesture** — every pointer gesture has a paired keyboard test in `EquipmentPanel.test.jsx`. *(Caveat added 2026-08-17 per `/sdd:audit`, tracked by #472's sibling area — six defects found against this requirement's own text: the narrow-arrangement arrow mapping is a row-major transpose rather than this requirement's column-major model, wrapping across the rank break the requirement forbids (`gridMove.js`, and pinned wrong by the test suite itself); no grab/target/commit announcements exist beyond rejection; focus is not moved to the destination after a successful move and is lost to `document.body`; Enter does not toggle an empty cell's blocked state because `preventDefault` fires unconditionally; stack continuation cells beyond the first are removed from the tab order; and the grid carries no accessible name or positional set. See the "Focus and Announcement" and "Accessibility Requirements" sections below, both of which this bullet's "built and tested" claim overstated.)*

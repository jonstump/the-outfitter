...
Backwater Outfitters persists saved loadouts server-side in a `lowdb` JSON file, scoped to a client-issued token carried in the `x-loadout-token` header. `SavedLoadoutsPanel` rendered every record the fetch returns in one flat column — this capability has since replaced it with `client/src/components/LoadoutListsPanel/LoadoutListsPanel.jsx`. Past roughly a dozen saves that became unusable, and users compensated by prefixing names by hand — "Rat — long ammo", "Rat — shotgun" — which is a grouping mechanism implemented in a text field.
...
*(added 2026-08-17, feedback item "loadouts under lists could be dragged and dropped to change the order of them")*

**Choice**: Each loadout carries an integer `order`, scoped to the list it is filed in (Unassigned included — it is a peer scope, not an exception). Cards within an open list or Unassigned render sorted by `order`. Reordering is drag-and-drop with a full keyboard equivalent, mirroring the pointer-drag-plus-keyboard-grab pattern SPEC-0006 already built for the equipment grid (`EquipmentSlot.jsx`/`EquipmentPanel.jsx`): grab a card (pointer-down on a handle, or Space/Enter on it when focused), move it with the pointer or Arrow Up/Down, drop with pointer-up or Space/Enter, cancel with Escape. The move is announced on a live region, the same channel `equip-announcer` already uses for the grid.

**This is deliberately NOT modelled on "Moving a loadout between lists is a native select, not a menu"**, three sections above, even though the two look like siblings. That decision governs *which list a loadout is filed into* — a small, enumerable set of destinations a `<select>` names perfectly. This decision governs *a loadout's position among its list-mates* — there is no discrete "position" to name in a menu; the only controls that express "before this card, after that one" without either a pointer or a large stack of "move to position N" options are drag-and-drop and up/down step buttons. Both were real options here (see Alternatives); drag was chosen because item feedback asked for it by name and the equipment grid had already paid the cost of building an accessible version once.

**Persisted server-side, unlike list sort order.** "List Ordering and Sorting" is explicit that a user's chosen SORT is client state and MUST NOT reach the server — sort is a lens the user can put down and pick back up, recomputed from data that already has its own truth (name, hunter, count, date). A drag-reordered card position has no such truth to recompute from: it is not derived from anything else the record holds, so if it lived only in `localStorage` it would evaporate on a new device or a cleared cache, and a "reorder" that only sometimes sticks is worse than the plain creation-order the app has today. The two decisions read as opposites only because they answer different questions — "how do I want to look at this, right now" versus "where did I put this thing" — and this codebase already treats those as different classes of state (the selected list is the same distinction, drawn the other way, in "The selected list is client state" below).

**Data model**: `order` is an integer, unique among loadouts sharing one `listId` (`null` included as a value, matching how the field already works everywhere else in this collection). No migration is needed for records that predate the field: a record with no stored `order` is *computed* one at read time — its position among its live, owned list-mates in the array's own storage order, which is the creation order every one of today's records already renders in. That computed value is never written back merely from being read, so nothing needs a backfill.

Three writers, all server-side, none trusting a client-chosen integer: the reorder endpoint below assigns `order = index` for every id in a client-supplied full list, after validating the list names exactly the caller's own loadouts in that scope; the existing move-between-lists write (`PATCH /api/loadouts/:id`, the `listId` field) assigns a freshly *computed* append value — one past the highest order (stored or computed) among the destination's current members — whenever `listId` actually changes; and record *creation* (`POST /api/loadouts`, the branch that inserts a brand-new record rather than upserting an existing one) assigns that same computed append value into whichever scope the new record is filed into, rather than leaving `order` unset for the read-time fallback to cover.

That third writer is not a nicety — an earlier revision of this feature left creation unassigned, reasoning that a freshly-pushed record is always last in storage order and so needs no explicit value. That reasoning holds only for a scope NOTHING has ever reordered. Once a scope has been reordered and then partly emptied (an older member deleted, or moved to a different list), its survivors keep whatever real `order` the reorder gave them — values that can be arbitrarily larger than the shrunk scope's new size. A record created afterward with no explicit `order` would fall back to a raw position among its live scope-mates in STORAGE order, which has no idea those larger values exist, and could render the newest loadout AHEAD of older, deliberately-arranged ones. Every entry point that can introduce a member into a list scope — create and move alike — now goes through the identical "one past the real maximum in scope" computation, so the array-position fallback is reachable only by a record that predates the requirement outright, which is exactly the case it exists to cover. See `server/src/routes/reorder.test.js` for the regression this closed.

All three writers are internal computations; the client never supplies a raw integer for anything but its own drag-produced full order to the reorder endpoint.

**Reorder is one request, not one PATCH per moved card.** A drag that displaces three cards to make room does not send three writes — the client computes the full ordered id list for the affected scope (one list, or Unassigned) once the drop lands, and sends it in a single call: `PATCH /api/loadouts/reorder`, body `{ listId, order: [id, id, …] }`. The server validates that `order` is *exactly* the set of live loadouts the caller owns in that `listId` scope — not a superset, not a subset — then writes `order: index` onto each named record in one pass. Anything else (a missing id, a foreign id, a duplicate, an id actually filed elsewhere) is a 400 naming the mismatch, never a partial write and never a silent drop — the same "reject loudly" instinct "Cross-collection ownership is checked explicitly" already applies to a listId reference, extended here to a whole reordering request instead of one write.

**Moving between lists appends, it does not splice.** When the existing move-select or `moveSaved` files a loadout into a new list, it lands at the end of that list's current order (`max(order) + 1` there, or `0` into an empty list/Unassigned) rather than at some position implied by where it used to sit. A moved card arriving at a specific spot in someone else's arrangement would be a second way to write `order` outside the one endpoint built to validate it, and "end of the list" needs no such validation because it can never collide.

**Rationale for drag-plus-keyboard over the alternatives**: see "Alternatives considered" below. In short, up/down buttons are simpler and keyboard-accessible for free, but the equipment grid already proved out a fully accessible drag pattern in this codebase — grab, arrow-step, drop, cancel, live-region announcement — so reusing it here costs the wiring, not the design, and it is what was actually asked for.

**Alternatives considered**:
- *Move up / move down buttons only*: rejected as the ONLY affordance — simpler and keyboard-native by construction, but it is not the drag interaction the feedback asked for, and a list of any length turns a far move into a long run of clicks. Kept in mind as the fallback shape if the keyboard equivalent below ever proves too costly to maintain alongside the grid's own.
- *Client-only order, like list sort*: rejected — see "Persisted server-side" above; a manually-built arrangement that doesn't survive a reload isn't the feature that was asked for.
- *A `loadoutList.cardOrder: string[]` array on the list record instead of a per-loadout field*: rejected for Unassigned, which has no list record to hold it — a second, parallel order-storage mechanism for one pseudo-list is more surface than a field every loadout already has room for. It also requires rewriting the whole array on every filing change instead of one integer.
- *Fractional/"between" ordering (assign each moved card a value between its new neighbours, no rewrite of the rest)*: rejected — it avoids the one-request-per-drop rewrite, but it accumulates precision problems after enough reorders and reads as clever rather than simple for a feature this size; the full-list PATCH is one request either way once a drop lands, so the complexity buys nothing here that a plain integer reindex doesn't already have.
...
- **Tightened validation now diverges further from SPEC-0006's wire v2** → The allowlist, the exact tuple bounds, and the `b` type check each reject the v2 shape that spec defines (nullable equipment entries, `b` as an array). Not a regression — v2 was already rejected and SPEC-0006 records itself as unimplemented — but the constraint now lives in three places in one validator with no cross-reference. Whoever implements SPEC-0006 must touch all three; a pointer beside the allowlist constant is the cheapest guard. *(**Closed 2026-08-15.** The risk landed and was paid down. `isValidData` now branches on `data.v` and accepts v1, v2 and v3 side by side, with the equipment grid and blocked array gated on `>= 2` and the weapon slot's exact element count gated per version. The "three places with no cross-reference" is **partly** resolved, and by the means this predicted. The two branches v3 touched — the version dispatch and the weapon-slot element count — now carry governing comments naming ADR-0023 and the SPEC-0009 requirements they implement. The equipment-grid branches cite ADR-0009 in prose but name no requirement, and the `DATA_KEYS` allowlist constant still has no pointer to SPEC-0006 — the "cheapest guard" this risk asked for is therefore still owed on the parts v3 did not have to touch. The exact-tuple rule this spec states is unchanged and still binds; what moved is that "exact" now resolves against the payload's declared version, which is recorded in this spec's own "A Write Stores Only What the Wire Format Defines".)* *(2026-08-16, per SPEC-0010 and issue #348: a fourth version landed the same way. `isValidData` now accepts v1 through v4 side by side; v4 touches only the weapon slot's `ammo` element, narrowing its type from integer to a two-entry id array rather than changing the element count again. The governing-comment gap this note found for v3 did not repeat — `isIslandV4`/`isAmmoSlotArray` in `server/src/routes/loadouts.js` cite SPEC-0010 by requirement name. The `DATA_KEYS` allowlist pointer to SPEC-0006 remains the one item still owed.)*

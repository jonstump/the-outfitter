...
Two writers, both server-side, neither trusting a client-chosen value: the reorder endpoint below assigns `order = index` for every id in a client-supplied full list, after validating the list names exactly the caller's own loadouts in that scope; and the existing move-between-lists write (`PATCH /api/loadouts/:id`, the `listId` field) assigns a freshly *computed* append value — one past the highest order (stored or computed) among the destination's current members — whenever `listId` actually changes. The second writer exists because neither "leave `order` alone" nor "reuse the record's array position" would reliably land the moved card at the end: a value carried over from its old list is a coincidence in its new one, and the record's fixed position in the underlying array reflects when it was *created*, not when it was *moved*, so a card moved into a newer list could sort into the middle of it by accident. Both writers are internal computations; the client never supplies a raw integer for anyone but its own drag-produced full order.

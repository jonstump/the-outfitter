...
Saved loadouts today are a flat list. A record is `{ id, owner, name, data, updatedAt }` in `server/data/db.json`, scoped to a client-issued token (issue #17), and `SavedLoadoutsPanel` (since superseded by `LoadoutListsPanel`, per this ADR's own decision) rendered every one of them in a single undifferentiated column. Once a user has more than a handful, the only organizing tool is the name field — people end up prefixing names by hand ("Rat — long ammo", "Rat — shotgun") to fake the grouping they actually want.

...
* **SPEC-0003 will need a small amendment.** Its dataset contract says the dataset provides "a portrait asset" — singular. Two sizes and the cross-size fallback rule are not expressible in that wording. This is an additive change to one requirement and its scenarios, not a rework, and should land alongside the spec for this dataset rather than being left to drift.

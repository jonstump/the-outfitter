...
- **List card** (poster style, one per list): `<button>`, height 220px, `position:relative`, background #17130c (`--scroll-track`), **3px solid border in the list's accent color** — the accent frame is the list's distinguishing identity per spec.
  ...
  - **Card states keep off the border, all of them (issue #132, tightened in PR #180's review).** The border is the accent and nothing else may write to it. Open = a 3px `--gold` ring drawn *inboard* of the frame by `.ll-card-open::after` — a pseudo-element, not an inset `box-shadow`, because an inset shadow paints below positioned descendants and the bottom third of it sat under the name plate. Hover = `filter: brightness(1.15)`, which reads over a portrait where a background change would not. Focus = 3px `--gold` outline at 2px offset, *outboard* of the frame. The three compose and stay tellable apart; hover and focus used to set `border-color`, at a higher specificity than `.ll-card`, so the open card — the one the user is concentrating on — lost its identity the moment it was pointed at or tabbed to.
...
- Footer row: "NAME" label (13px ls 1px muted) · text input (placeholder "defaults to the hunter's name"; picking a hunter fills the input with the hunter's name unless the user already typed) · **accent picker** · "Create list" (`.btn-primary` red) · Cancel.
  - **Changed 2026-08-10 (issue #135):** this was an 18×18 read-only *preview* swatch of the value the server was about to auto-assign. A colour beside a name field reads as choosable, and it was not. It is now the same `AccentPicker` the expanded header uses — one `role="radiogroup"` of six **24×24** swatches, each labelled with its colour name — pre-selected to the least-used palette value, so a user who ignores it still gets exactly the old behaviour. The chosen value rides on `POST /api/loadout-lists`; there is no post-create PATCH. Its accessible name is "Accent colour for the new list", since there is no list to name yet.
  - **Corrected 2026-08-10 (PR #180 review):** this entry read "arrow-key navigable" as delivered behaviour when the control had no key handler at all — arrow keys are free for `<input type="radio">`, not for a `<button role="radio">`. The keyboard model is now implemented rather than assumed, following `HunterPicker`'s roving-tabindex idiom: **one tab stop** (the checked swatch, or the first when the stored value is off-palette), Left/Up and Right/Down move *and* select with wrapping, Home/End jump to the ends, Space/Enter select. Swatches were also raised 18×18 → **24×24**: SC 2.5.8's minimum is 24px, this file and `global.css` both cite it, and #135 took the create form from zero targets to six.
  - **States, three separate channels** (WCAG 2.4.7): checked = `--gold-bright` border plus a 2px inset `--panel` ring; hover = `--gold-bright` border; focus = 2px `--gold-bright` outline at 2px offset. Focus previously changed nothing at all on the checked swatch, which is the one the first Tab lands on.
